...
<body>
<div class="loginBox">
    <form action="/signUp" method="post" name="signUp_form" id="frm"
...
<script>
    ...
    $("#id").on("change", function () {
        ...
        $.ajax({
            ...
            if (res) {
                $(".idDupleCheck").text("중복된 아이디 입니다.");
                idDupleCheck = false;
            } else {
                $(".idDupleCheck").text("사용가능한 아이디 입니다.");
                idDupleCheck = true;
            }
        });
    ...
    function signUpRegex() {
        ...
        if ($("#authenticNum").val() != msg) {
            $("#checkAuthenticNum").text("인증번호가 일치하지 않습니다.");
        ...
        if (regexId.test(id)==false) {
            $("#checkId").text("아이디 형식이 맞지 않습니다.");
            return false;
        }

        //pw 유효성
        if (regexPw.test(pw)==false) {
            $("#checkPw").text("비밀번호 형식이 맞지 않습니다.");
            return false;
        }
        //pw 불일치
        if (pwOk == false) {
            $("#equalPw").text("비밀번호가 일치하지 않습니다.");
            return false;
        }

        //이름 유효성
        if (regexName.test(name)==false) {
            $("checkName").text("이름 형식이 맞지 않습니다.");
            return false;
        }
        //이메일 유효성
        if (regexEmail.test(email)==false) {
            $("#checkEmail").text("이메일 형식이 맞지 않습니다.");
            return false;
        }
        //전화번호 유효성
